...
CREATE PROCEDURE `sp_store_image`(IN `case_id` BIGINT(20), IN `attribute_id` CHAR(24) CHARSET utf8, IN `new_value` TINYTEXT CHARSET utf8, IN `user_email_hash` CHAR(128) CHARSET utf8)
begin
    ...
    if (user_int is not null) THEN
        insert into image_values (at, attribute_id, case_id, user, value) values (
            now(),
            attribute_id,
            case_id,
            user_int,
